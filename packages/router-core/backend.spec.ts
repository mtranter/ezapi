...
describe("BackendUtils", () => {
  describe("urlParser", () => {
    ...
    it("should reject non greedy patern", () => {
      ...
      const parser = BackendUtils.urlParser();
      const result = parser.parseUrl("/courses/{id}", `/courses/create/status/52d8c767-e281-4a27-bbc5-7cdf91a76672`);
    ...
    it("should parse url with greedy param", () => {
      const dummyUuid = "12345678-1234-1234-1234-123456789012";
      const parser = BackendUtils.urlParser();
      const result = parser.parseUrl("/courses/{id+}", `/courses/create/status/${dummyUuid}`);
      expect(result.isMatch).toBe(true);
    ...
    it("should parse url with normal and greedy param", () => {
        const dummyUuid = "12345678-1234-1234-1234-123456789012";
        const parser = BackendUtils.urlParser();
        const result = parser.parseUrl("/courses/{action}/status/{id+}", `/courses/create/status/${dummyUuid}/my-id`);
        expect(result.isMatch).toBe(true);
